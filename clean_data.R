...
rh_data = rh_data[,c(2:7,9:12,15,19,22,23)]
lh_data = lh_data[,c(2:7,9:12,15,19,22,23)]
...
# Perform mean imputation for missing intracranial volumes.
rh_mean_icv = mean(rh_data$ICV, na.rm = T)
rh_data$ICV[is.nan(rh_data$ICV)] = rh_mean_icv

lh_mean_icv = mean(lh_data$ICV, na.rm = T)
lh_data$ICV[is.nan(lh_data$ICV)] = lh_mean_icv

# Visualize standardized data.
z_score <- function(x) {(x - mean(x, na.rm = T))/sd(x, na.rm = T)}
  
z_rh_data = apply(rh_data[,2:14],2,z_score)
z_lh_data = apply(lh_data[,2:14],2,z_score)
